...
def solve_and_sim_reiter(vec):
    #solve
    # Parameters and setup
    vecin = vec.numpy().reshape(-1)
    beta = 0.97
    gamma = 2.0
    rho = vecin[0]#0.95
    rho_xi = vecin[1]#0.80
    ...
    xvec = xvec[1:,:].reshape((1,nX*iterations))
    # 
...
def likelihood_reiter(vec, xvalue, ranvar):
    ...
    zscorevec[0,:] = Qinv@xvalue[0,:]*100
    for idx in range(1,iterations):
        zscorevec[idx,:] = Qinv@(xvalue[idx,:]  - P@xvalue[idx-1,:])*100
    llike = np.sum(np.log(ranvar.pdf(zscorevec)))
...
xdata = np.loadtxt('reiter_RBC.csv', delimiter = ',').reshape((61,812))
#print(xdata.shape)
...
# class rw_metroplolis_sampler:
#     def __init__(self, logpdf, initial_position):
#         self.logpdf = logpdf
#         self.pos = initial_position
    
def kernelfunct(logpdf, initial_position, iterations):
    ...
    while True:
        try:
            log_prob = logpdf(position_old)
            move_proposals = np.random.multivariate_normal(0.*prior_max,np.square(np.diag((prior_max - prior_min)/100)))
            proposal1 = position_old + move_proposals
            # print()
            #print(move_proposals)
            #in1s = input("press enter")
            proposal2 = np.where(proposal1 < prior_max, proposal1, prior_max)
            proposal = np.where(proposal2 > prior_min, proposal2, prior_min)
            proposal_log_prob = logpdf(proposal)
            
            log_uniform = np.log(np.random.rand())#initial_position.shape[0], initial_position.shape[1]
            do_accept = log_uniform < -proposal_log_prob + log_prob
            
            position = np.where(do_accept, proposal, position_old)
            #self.log_prob = np.where(do_accept, proposal_log_prob, self.log_prob)
    ...
    # while True:
    #     try:
    #         move_proposals = np.random.multivariate_normal(0.*prior_max,np.square(np.diag((prior_max - prior_min)/30)))
    #         proposal1 = position + move_proposals
    #         # print()
    #         #print(move_proposals)
    #         #in1s = input("press enter")
    #         proposal2 = np.where(proposal1 < prior_max, proposal1, prior_max)
...
# def iter_mp():
#     model = rw_metroplolis_sampler(likewrapper, arraystart)
#     return model.kernelfunct(1000)
...
#processes = [mp.Process(target=iter_mh,args=(out,)) for i in range(20)
timevec = [[] for i in range(5)]
otime = time.time()
for i in range(5):
    pool.apply_async(kernelfunct, args = (likewrapper, arraystart, 1000), callback = log_result)
    timevec[i] = time.time()-otime
pool.close()
pool.join()
print(timevec)
...
# posarr = [[] for j in range(n_samples)]

# #for i in range(n_samples):

# posarr = model.kernelfunct(1000)

    


# adam_optimizer = AdamOptimizer(arraystart, alpha=0.01)

# adam_history = [] # to plot out path of descent
# adam_history.append(dict({'theta': arraystart, 'cost':  likewrapper(arraystart)}))

# for i in range(500):
#     # try:
#     for i in range(10):
#         if arraystart[i] > prior_max[i]:
#             arraystart[i] = prior_max[i]
#         elif arraystart[i] < prior_min[i]:
#             arraystart[i] = prior_min[i]
#     g = grad_like(arraystart)
#     arraystart = adam_optimizer.backward_pass(g)
#     adam_history.append(dict({'theta': arraystart, 'cost':  likewrapper(arraystart)}))
#     #arraystart = arraystart + .000005*g#.000035
#     #array_like[i] =  likewrapper(arratstart)

        
#     # except:
#     #     arraystart = arraystart + .01*np.random.randn(10)

# print(arraystart)
   
# sampnum = samples.numpy()
# np.savetxt('reiter_RBC.csv', sampnum, delimiter = ',')
...
# Linearize
#P is autoregressive matrix and Q is the SD of shocks
...
# IRF[:,0] = Q[:,1]  # Shock of size epsilon= 1 is implicit here #Q is 201 savings, 201 income (both G), 402 distributions of (something) D 9 Agg_SS variables
